# Agent Lens - 产品需求文档 (PRD)

## 1. 概述

### 1.1 产品定位

Agent Lens 是一个本地 CLI 工具，用于发现、聚合和可视化多种 AI Coding Agent 的本地会话记录。用户通过 `npx agent-lens` 或 `bunx agent-lens` 一键启动，在浏览器中按 Agent 类型和项目目录分组浏览所有会话，支持日期过滤和多维度筛选。
...
Agent Lens 将两者的能力整合为一个 TypeScript monorepo，提供从会话发现到可视化的端到端体验，无需 Python 环境。
...
npx agent-lens --agent codex,cursor

# 指定项目目录
npx agent-lens --cwd /path/to/project

# 指定时间范围
npx agent-lens --from 2026-04-01 --to 2026-04-15

# 指定具体会话（直接跳转）
npx agent-lens --session claude://session-id

# 指定端口
npx agent-lens --port 3000
...
│   │           └── sqlite.ts
│   │
│   └── cli/                     # @agent-lens/cli（npm bin: agent-lens）
│       └── src/
│           ├── index.ts         # CLI 入口
│           ├── commands/
...
1. `@agent-lens/core` → tsc 编译
2. `@agent-lens/web` → vite build（依赖 core 类型）
3. `@agent-lens/cli` → tsup 打包（依赖 core；将 web dist 嵌入到产物中）

**发布**：

仅发布 `agent-lens` CLI 包到 npm。CLI 包内嵌 Web 构建产物，用户 `npx agent-lens` 即可使用，无需额外安装。

```json
{
  "name": "agent-lens",
  "bin": { "agent-lens": "dist/cli.mjs" },
  "files": ["dist/"],
  "dependencies": {
