...
- **全文搜索** —— 按会话标题和对话内容检索，并高亮命中片段
- **Dashboard 与活跃趋势** —— 一眼看到每日活跃、Agent 分布、最近会话、最新活动时间、Token 用量和模型用量
- **会话收藏** —— 收藏重要会话，并在 Dashboard 中快速回看
- **完整对话回放** —— 原样回放每一条消息、工具调用和推理步骤
- **文件变更追踪** —— 从会话详情目录快速跳转到被读取、编辑、创建、删除或移动的文件
- **键盘导航** —— 通过快捷键切换视图、聚焦搜索、打开快捷键面板
- **成本与 Token 可见** —— 精确查看每个会话消耗了多少 Token 和费用
- **SQLite 缓存与搜索索引** —— 用统一的本地存储快速恢复列表并复用搜索索引
...
1. **Dashboard** —— 总会话数、总消息数、总 Token、最新活动、每日活跃趋势、Agent 分布、模型分布、Token 趋势、收藏会话和最近会话
2. **搜索** —— 在顶部搜索框中检索会话标题和对话内容，并直接跳转到高亮命中结果
3. **Agent 侧边栏** —— 列出所有检测到的 Agent 及其会话数量，点击任意 Agent 可筛选视图
4. **会话列表** —— 按最新时间排序浏览会话，每张卡片显示标题、工作目录、消息数和总成本
5. **会话收藏** —— 固定需要反复查看的会话，并在 Dashboard 或会话视图中管理
6. **会话详情** —— 点击任意会话查看完整回放，包括所有用户消息、Assistant 回复、工具调用、推理步骤、模型标签和文件变更
7. **快捷键** —— 通过快捷键面板查看导航、搜索和分组跳转操作
8. **实时同步** —— 服务运行期间，本地新增或更新的会话会自动反映到界面
