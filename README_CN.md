...
1. **Agent 侧边栏** —— 列出所有检测到的 Agent 及其会话数量，点击任意 Agent 可筛选视图
2. **会话列表** —— 按最新时间排序浏览会话，每张卡片显示标题、工作目录、消息数和总成本
3. **会话详情** —— 点击任意会话查看完整回放，包括所有用户消息、Assistant 回复、工具调用和推理步骤
4. **统计栏** —— 每个会话顶部显示总 Token 用量（输入 + 输出）和费用
